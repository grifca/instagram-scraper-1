          ...
          <div class="dropdown-menu">
            ...
            if($outputs) {
                ...
                foreach($outputs as $dir => $identifier) {
                    $identifier = ltrim($identifier, 'reports/');
                    if($reportCount == 0) {
                        echo '<a href="view.php?i='.$identifier.'" class="dropdown-item active">'.$identifier.'</a>';
                    } else {
...
if(isset($_GET['r'])) {
    $f = fopen("reports/".$_GET['i']."/report_".$_GET['i'], "r");
} else {
    $reports = glob('reports/'.$_GET['i'].'/*');
    // var_dump($reports[0]);
    // die();
    $f = fopen($reports[0], "r");
}
